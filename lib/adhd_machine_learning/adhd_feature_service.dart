...
class AdhdMlService {
  ...
  Future<void> initialize() async {
    ...
    if (isNativeInterpreterReady) {
      try {
        ...
        // FIX: Using lowercase keys consistent with Python script
        _ratingScaleMin = (definitions['RATING_MIN'] as num).toDouble();
        _ratingScaleMax = (definitions['RATING_MAX'] as num).toDouble();
        print(
            "✅ Definitions loaded. Rating scale [$_ratingScaleMin, $_ratingScaleMax]");
      ...
      print(
          "Failed to initialize native interpreter after $maxRetries attempts.");
      throw Exception(
  ...
  /// NEW METHOD: predictRecord
  /// Takes a Records object, preprocesses its data, and then calls the main `predict` method.
  Future<Map<String, double>> predictRecord(Records record) async {
    ...
    // 1. Preprocess data from the Record object
    ...
    final List<int> processedKeywords = publicTokenizeText(
      text: keywordString,
      vocab: _keywordVocab, // Use internal field directly
      sequenceLength: _keywordSequenceLength, // Use internal field
    );

    final String contentString = "${record.title ?? ''} ${record.content ?? ''}";
    final List<int> processedContent = publicTokenizeText(
      text: contentString,
      vocab: _contentVocab, // Use internal field
      sequenceLength: _contentSequenceLength, // Use internal field
    );
    ...
    print("AdhdMlService.predictRecord - Preprocessed Keywords (len: ${processedKeywords.length}): $processedKeywords");
    print("AdhdMlService.predictRecord - Preprocessed Content (len: ${processedContent.length}): $processedContent");
  ...
  Future<Map<String, double>> predict(  Records record ,{
    required List<int> keywords, required List<int> content, required List<double> rating}) async {
    if (!_isInitialized) {
