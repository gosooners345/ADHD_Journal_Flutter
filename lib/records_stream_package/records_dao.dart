...
class RecordsDao{
  ...
  Future<List<Records>> getSearchedRecords({required String query}) async{
      List<String> columns = ['tags','title','content','emotions','sources','symptoms'];
      final database = await recordsDB.database;
      List<Map<String,dynamic>> maps;

        if(query.isNotEmpty) {
          maps = await database.query('records',where: /*title LIKE ?*/
              "title like ?  OR "
          "emotions LIKE ? OR  "
              "sources LIKE ? OR "
              "content LIKE ? OR "
          'symptoms LIKE ? OR '
              "tags LIKE ?",whereArgs: ['%$query%','%$query%','%$query%','%$query%','%$query%','%$query%']);}
        else{
...
String orderType = '';
    switch(type){
      case 'Alphabetical' : columnName = "title"; orderType='ASC';break;
      case 'Most Recent' : columnName = 'time_Updated'; orderType= 'DESC'; break;
      case 'Time Created': columnName = 'time_Created'; orderType='ASC'; break;
      case 'Rating' : columnName= 'rating';orderType='DESC';break;
    }


      final database = await recordsDB.database;
    final List<Map<String, dynamic>> maps = await database.rawQuery("SELECT * FROM records ORDER BY $columnName $orderType;");
